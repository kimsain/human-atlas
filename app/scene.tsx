...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError('이 브라우저에서 3D 뷰어를 시작하지 못했습니다. WebGL을 사용할 수 있는 브라우저에서 다시 시도하세요.');return;}
  ...
  renderer.domElement.setAttribute('aria-label','인터랙티브 인체 해부학 모델입니다. 드래그하여 회전하고, 손가락을 벌리거나 오므리거나 스크롤하여 확대·축소하고, 구조를 탭하여 상세 정보를 확인하세요.');
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('해부학 지오메트리를 조립하지 못했습니다.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:'해부학 모델을 불러오지 못했습니다.');}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError('기기에서 3D 세션이 중지되었습니다. 계속하려면 뷰어를 다시 불러오세요.');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
